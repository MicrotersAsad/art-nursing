...
const Achievement = () => {
  ...
  useEffect(() => {
    async function fetchPhotos() {
      try {
        const response = await fetch('/api/achievement'); // Make sure the API endpoint is correct
    ...
    <div className="max-w-7xl mx-auto bg-white">
      <div className="photo-gallery grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 p-4">
        {photos.map((photo, index) => (
          <div key={index} className="photo-item border rounded shadow p-3">
            <Zoom>
              <Image
                src={photo.imagePath} // Ensure this is the correct path to your photo
                alt={photo.alt || 'Photo'}
                className="w-[400px] h-[200px] object-cover cursor-pointer" // Fixed width and height
                width={400}
                height={400}
            ...
            <h3 className="mt-2 text-center text-lg font-semibold text-gray-800">
              {photo.title || 'No Title'} {/* Display title below image */}
            </h3>
          </div>
        ))}
