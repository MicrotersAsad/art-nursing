...
export default function ProgrammeDetails() {
  ...
  }, [program]); // Re-apply when program details are loaded
  if (loading) {
...
<hr className='mt-3 mb-3'/>
        ...
        <ul className="space-y-2">
          ...
          {notices.map((notice, index) => (
            <li key={index}>
              <a href={`/notices/${notice.slug}`} className="text-green-600 hover:underline sm:text-sm">
                {notice.title}
              </a>
            </li>
          ))}
...
<section id="faculty-members" className="mb-8 border rounded p-4 text-center">
        ...
        <section id="tuition-fees" className="mb-8 border rounded p-4 jodit-content">
          ...
          <div className="text-gray-700 sm:text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: program.tuitionFees }} />
           ...
           <section id="dean-message" className="mb-8 border rounded p-4">
            ...
            <div className="text-center">
              <img src={program.deanImage} alt={program.deanName} className="w-32 h-32 object-cover mx-auto rounded-full mb-4" />
              <h3 className="font-semibold text-lg">Name: {program.deanName}</h3>
              <p className="text-gray-500">Mesage:   {program.deanMessage}</p>
            </div>
          </section>
          ...
          <section id="head-message" className="mb-8 border rounded p-4">
            ...
            <div className="text-center">
              <img src={program.headImage} alt={program.headName} className="w-32 h-32 object-cover mx-auto rounded-full mb-4" />
              <h3 className="font-semibold text-lg">Name: {program.headName}</h3>
              <blockquote className="text-gray-500">Mesage:    {program.headMessage}</blockquote>
            </div>
          </section>
      </div>
