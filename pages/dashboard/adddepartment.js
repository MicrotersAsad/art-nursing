...
function Department() {
  ...
  const fetchdepartment = async () => {
    try {
      ...
      setdepartment(data || []);  // Safeguard against undefined data
    } catch (error) {
  ...
  const handleSaveCategory = async () => {
    ...
    const response = await fetch(`/api/department${editingCategory ? `?id=${editingCategory._id}` : ''}`, {
      method,
      headers: {
    ...
    if (response.ok) {
      ...
      await fetchdepartment(); // Reload department after saving
    } else {
  ...
  const handleDeleteCategory = async (id) => {
    try {
      ...
      await fetchdepartment(); // Update the category list after deletion
  ...
  return (
    <Layout>
      <div className="container mx-auto p-5">
        <h2 className="text-2xl font-bold mb-5">department</h2>
        ...
        <h3 className="text-xl font-semibold mb-3">department List</h3>
