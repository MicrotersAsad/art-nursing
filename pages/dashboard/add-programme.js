...
export default function AddProgram() {
  ...
  const [facultyMembers, setFacultyMembers] = useState([{ name: '', designation: '', image: null, imageUrl: '' }]);
  const [deanImage, setDeanImage] = useState(null);
  ...
  const handleAddFaculty = () => {
    setFacultyMembers([...facultyMembers, { name: '', designation: '', image: null, imageUrl: '' }]);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (deanImage) {
      ...
      formData.append('deanImageUrl', deanImageUrl);
    }

    if (headImage) {
      formData.append('headImage', headImage);
    } else {
      formData.append('headImageUrl', headImageUrl);
    }

    // Convert facultyMembers to JSON and append
    formData.append('facultyMembers', JSON.stringify(facultyMembers));

    // Append each faculty member's image separately
  ...
  const resetForm = () => {
    ...
    setHeadName('');
    setHeadMessage('');
    setFacultyMembers([{ name: '', designation: '', image: null, imageUrl: '' }]);
    setDeanImage(null);
    setHeadImage(null);
  };
  ...
  const handleEdit = async (slug) => {
    ...
    try {
      ...
      setDeanImageUrl(program.deanImage || '');
      setHeadImageUrl(program.headImage || '');
      ...
      <div className="container mx-auto p-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className='mb-3 border rounded p-5'>
            ...
            <JoditWrapper initialContent={salientFeatures} onChange={setSalientFeatures} />
          </div>

          {/* Curriculum Structure */}
          <div className='mb-3 border rounded p-5'>
            <label className="block mb-2 font-semibold">Add Curriculum Structure</label>
            <JoditWrapper initialContent={curriculumStructure} onChange={setCurriculumStructure} />
          </div>

          {/* Eligibility Requirements */}
          <div className='mb-3 border rounded p-5'>
            <label className="block mb-2 font-semibold">Add Eligibility Requirements</label>
            <JoditWrapper initialContent={eligibilityRequirements} onChange={setEligibilityRequirements} />
          </div>

          {/* Tuition Fees */}
          <div className='mb-3 border rounded p-5'>
            <label className="block mb-2 font-semibold">Add Tuition Fees</label>
            <JoditWrapper initialContent={tuitionFees} onChange={setTuitionFees} />
          </div>
